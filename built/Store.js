...
exports.showTyping = function () { return ({
    type: 'Show_Typing',
    activity: {
        type: "typing",
        from: {
...
exports.shell = function (state, action) {
    ...
    switch (action.type) {
        ...
        case 'Send_Message':
            return tslib_1.__assign({}, state, { input: '', processingMessage: true });
        case 'Recv_Message':
            return tslib_1.__assign({}, state, { processingMessage: false });
        case 'Set_Send_Typing':
            return tslib_1.__assign({}, state, { sendTyping: action.sendTyping });
        case 'Card_Action_Clicked':
            return tslib_1.__assign({}, state, { processingMessage: true, lastInputViaSpeech: false });
        default:
            return state;
    }
};
...
exports.resetActivities = function () { return ({
...
exports.history = function (state, action) {
    ...
    switch (action.type) {
        ...
        case 'Receive_Message':
            var isTemp = (action.activity.type === 'message' && action.activity.entities && !action.activity.entities[0].showMessage);
            // case of message part of group of messages and it is not the last one
            if (isTemp) {
                if (state.activitiesTemp.find(function (a) { return a.id === action.activity.id; }))
                    return state; // don't allow duplicate messages
                //store the activity in temp Array and return state
                return tslib_1.__assign({}, state, { activitiesTemp: state.activitiesTemp.filter(function (activity) { return activity.type !== "typing"; }).concat([
                        action.activity
                    ], state.activitiesTemp.filter(function (activity) { return activity.from.id !== action.activity.from.id && activity.type === "typing"; })) });
            }
            // case of message not temporary
            // if exists previous temp activities we pass them to final activities
            for (var i_2 = 0; i_2 < state.activitiesTemp.length; i_2++) {
                state.activities.push(state.activitiesTemp[i_2]);
            }
            ...
            return tslib_1.__assign({}, state, { activities: state.activities.filter(function (activity) { return activity.type !== "typing"; }).concat([
                    action.activity
                ], state.activities.filter(function (activity) { return activity.from.id !== action.activity.from.id && activity.type === "typing"; })), activitiesTemp: [] });
        case 'Send_Message':
            return tslib_1.__assign({}, state, { activities: state.activities.filter(function (activity) { return activity.type !== "typing"; }).concat([
        ...
        case 'Send_Message_Retry': {
            ...
            return tslib_1.__assign({}, state, { processingMessage: false, activities: state.activities.filter(function (activityT) { return activityT.type !== "typing" && activityT !== activity_2; }).concat([
                    newActivity_1
        ...
        case 'Send_Message_Fail': {
            var i_3 = state.activities.findIndex(function (activity) {
                return activity.channelData && activity.channelData.clientActivityId === action.clientActivityId;
            });
            if (i_3 === -1)
                return state;
            var activity_3 = state.activities[i_3];
            ...
            return tslib_1.__assign({}, state, { activities: copyArrayWithUpdatedItem(state.activities, i_3, newActivity_2), clientActivityCounter: state.clientActivityCounter + 1, selectedActivity: state.selectedActivity === activity_3 ? newActivity_2 : state.selectedActivity });
        }
        case 'Show_Typing':
        ...
        case 'Message_Failure':
            return tslib_1.__assign({}, state, { activities: state.activities.concat([action.activity]) });
        case 'Clear_Typing':
        ...
        case 'Reset_Activities':
            return tslib_1.__assign({}, state, { activities: [state.activities[0]] });
        default:
            return state;
    }
};
...
exports.setActions = function (actions, innerActions) { return ({
    type: 'Set_Actions',
    actions: actions,
    innerActions: innerActions
}); };
exports.hideActionsPanel = function () { return ({
    type: 'Hide_Panel'
}); };
exports.showActionsPanel = function () { return ({
    type: 'Show_Panel'
}); };
exports.toggleInnerPanel = function () { return ({
    type: 'Toggle_Inner_Panel'
}); };
exports.freqActions = function (state, action) {
    if (state === void 0) { state = {
        actions: [],
        innerActions: [],
        showPanel: true,
    ...
    switch (action.type) {
        case 'Hide_Panel':
            return tslib_1.__assign({}, state, { showPanel: !state.showPanel });
        case 'Toggle_Inner_Panel':
            return tslib_1.__assign({}, state, { showInnerPanel: !state.showInnerPanel });
        case 'Show_Panel':
            return tslib_1.__assign({}, state, { showPanel: true });
        case 'Set_Actions':
            return tslib_1.__assign({}, state, { actions: action.actions, innerActions: action.innerActions });
        default:
            return state;
    }
};
exports.logoWidergy = function (state, action) {
    if (state === void 0) { state = {
        showLogoWidergy: true
    }; }
    switch (action.type) {
        case 'Show_Logo':
            return tslib_1.__assign({}, state, { showLogoWidergy: true });
        default:
            return state;
    }
};
